...
##### ① 전송계층과 응용계층사이에 존재하는 저수준 API 즉 인터페이스다

* 응용계층은 반드시 데이터 송수신을 위해 소켓을 통해 전송계층으로 보내야한다.
  즉, 네트워크상 종단기기(EndPoint)와 TCP,UDP 포트와 연결을 시켜주는 인터페이스 
* 이를 통해 네트워크 환경에서 IPC를 진행한다
* REST / RPC 같은 네트워크 API를 구현하는데 사용하기도 한다.

##### ② 프로토콜, IP, 포트번호로 구성된다.
* 프로토콜
  1. "TCP/IP" 프로토콜을 사용하는 Socket API가 있고.
  2. "UDP/IP" 프로토콜을 사용하는 Socket API도 있다.
* IP 주소 : 호스트마다 하나씩 존재 (서버장비, 네트워크 장비)를 표현
* 포트번호 : 어플리케이션

##### ③ REST API와 차이점은 
  * HTTP 문서를 Keep-alive 형식으로 HTTP 레이어에서 세션을 유지
  * 양방향 통신이 가능하다
